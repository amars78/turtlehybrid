...
# 2. 포트폴리오 설정
tickers_input = st.sidebar.text_area("관심 종목 리스트 (쉼표로 구분)", "AAPL, MSFT, NVDA, TSLA, 005930.KS")
...
with tab1:
    ...
    for i, ticker in enumerate(tickers):
        ...
        my_bar.progress((i + 1) / len(tickers), text=f"{ticker} 분석 중...")
...
with tab2:
    ...
    selected_ticker = st.selectbox("분석할 종목을 선택하세요", tickers)
    ...
    if df_chart is not None and not df_chart.empty:
        ...
        fig.update_layout(title=f'{selected_ticker} CAN SLIM x 터틀 분석 차트', xaxis_rangeslider_visible=False, template='plotly_white', height=600)
